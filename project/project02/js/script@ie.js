$(function () {
    ...
    //실시간 검색어
    var list = $('.keyword_overlay ol li');
    var current = 0;
    function keywordMove() {
        var keyword = setInterval(function () {
            var prev = list.eq(current);
            move(prev, 0, '-100%');
            current++;
            if (current >= list.size()) {
                current = 0;
            };
            var next = list.eq(current);
            move(next, '100%', 0);
        }, 3000);
    ...
    $('.new_album_artist button').click(function () {
        $(this).toggleClass('on');
        var state = $(this).find('+.popup').css('display');
        if (state === 'none') {
    ...
    var subList = $('.sub_list'),
        leftBtn = $('.new_album .slider_btn a.prev'),
        rightBtn = $('.new_album .slider_btn a.next'),
        menuBtn = $('.new_album_cate li'),
        menuBtn01 = $('.new_album_cate li.menu01'),
        menuBtn02 = $('.new_album_cate li.menu02'),
        menuBtn03 = $('.new_album_cate li.menu03'),
        pageNum = $('.new_album .page_num strong'),
        num = 0;
